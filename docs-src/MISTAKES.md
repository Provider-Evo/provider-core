...
>
> **活文档**：在工作过程中实时更新本文件。
> **最后更新：** 2026-05-16T18:50:00+08:00
...
**实际情况：** 无任何构建元数据。`main.py` 通过 `sys.path.insert(0, str(_ROOT))` 手动注入路径
**犯错后果：** `pip install -e .` 或 `python -m src.main` 都会失败
**正确做法：** 只能从项目根目录执行 `python main.py`
...
**澄清：** 实际运行两个 Python 进程。Runner 监控 Worker 退出，exit code 42 触发自动重启（最多 50 次）
