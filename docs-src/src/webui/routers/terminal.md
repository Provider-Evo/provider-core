...
xterm.js 终端支持三种背景模式，通过右键上下文菜单切换：
...
- **"自定义背景图片"**：允许用户上传自定义背景图片，支持透明度调节和填充模式选择。

### 自定义背景图片功能

当选择"自定义背景图片"模式时，终端区域会显示额外的控制组件：

- **图片选择按钮**：点击后打开文件选择器，支持常见的图片格式（PNG、JPG等）
- **透明度滑块**：调节背景图片的透明度（10%-90%），默认30%
- **填充模式按钮**：切换背景图片的填充模式（cover/contain/stretch），默认cover

背景图片通过 CSS 伪元素（`::before`）实现，确保终端内容始终显示在背景图片之上。图片数据以 Data URL 格式存储在 CSS 变量 `--custom-bg-image` 中，透明度存储在 `--custom-bg-opacity` 中。
...
终端面板使用统一的 TabBar 组件管理标签页。TabBar 支持水平顶部栏和垂直侧边栏两种布局。

### 标签页创建

- 点击加号按钮（"+"）调用 `_createChooserTab()`，创建一个新的选择器标签页
- 选择器标签页提供"本地终端"和"远程终端"两个按钮
- 点击任一按钮后，选择器标签页转换为实际终端标签页
- 标签页数量没有硬编码限制，可以无限创建

### 加号按钮

- 加号按钮始终可见，不受标签页数量限制
- 在水平布局中，加号按钮使用 `position: sticky; right: 0` 固定在标签栏右侧
- 在垂直布局中，加号按钮固定在侧边栏底部
- 加号按钮的 z-index 设置为 2，确保始终在最上层

### 关闭全部按钮

- 当标签页数量达到 6 个时（`closeAllThreshold` 配置），显示"关闭全部"浮动按钮
- 该按钮用于批量关闭所有标签页
- 在水平布局中，按钮定位在 `right: 40px`，避免遮挡加号按钮
- 在垂直布局中，按钮定位在侧边栏底部中央
