...
所有 zen 数据统一写入 `persist/zen/`：

| 文件 | 用途 |
|------|------|
| `models.json` | ModelsCache 模型列表 |
| `proxy_pool.json` | 代理池快照 |
| `proxy_score.json` | 代理汤普森采样分数 |

`ModelsCache` 的 `platform` 参数固定为 `"zen"`（与适配器 `name` 一致），不再使用 `zen-proxy` / `zen-key` 子目录。
