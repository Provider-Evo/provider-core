...
- **WebUI** — 内置 Web 管理界面，支持配置编辑、日志查看、平台状态监控
...
支持 Anthropic Claude API 格式的请求，包括 tool_use/tool_result 等特性。
...
```
客户端 → aiohttp.web → 中间件 (CORS/认证/错误/自定义) → 路由处理器 → 平台适配器 → AI 后端
                                    ↓
                              根据 model 字段自动选择平台
```
