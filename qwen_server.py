...
from script.qwen.qwen_client import *
from script.qwen.qwen_util import *
...
class QueueFullError(SchedulerError):
    ...
    def __init__(
        self, scheduler_name: str, queue_depth: int, max_queue: int
    ) -> None:
...
class SchedulerShutdownError(SchedulerError):
    ...
    def __init__(self, scheduler_name: str) -> None:
        self.scheduler_name = scheduler_name
        super().__init__(
            f"调度器 '{scheduler_name}' 已关闭，不再接受新请求"
        )
        ...
        async with self._lock:
            ...
            if current_queue >= self._max_queue_size:
                ...
                raise QueueFullError(
                    self._name, current_queue, self._max_queue_size
                )
...
class SchedulerConfig:
    ...
    MEDIA_MAX_CONCURRENT: int = int(
        os.environ.get("SCHED_MEDIA_CONCURRENT", "10")
    )
    MEDIA_MAX_QUEUE: int = int(os.environ.get("SCHED_MEDIA_QUEUE", "100"))
    MEDIA_TIMEOUT: float = float(
        os.environ.get("SCHED_MEDIA_TIMEOUT", "180")
    )
...
class OpenWebUIModelCapabilities:
    """
    OpenWebUI 模型能力元数据构建器

    根据 OpenWebUI 规范构建标准的模型 info 字段，
    确保模型能被 OpenWebUI 正确分类和显示。

    OpenWebUI 通过 info.meta.capabilities 中的以下字段识别模型类型:
    - chat: bool     - 是否支持聊天补全
    - image: bool    - 是否支持图像生成
    - tts: bool      - 是否支持文本转语音
    - embedding: bool - 是否支持嵌入向量

    以及 info.meta.type 字段标识模型主要类型:
    - "llm"         - 大语言模型
    - "image"       - 图像生成模型
    - "tts"         - 文本转语音模型
    - "embedding"   - 嵌入模型
    ...
    def build_llm_info(
        ...
        info: Dict[str, Any] = {
            ...
            "meta": {
                ...
                "type": "multimodal" if supports_vision else "llm",
            },
        }
    ...
    @staticmethod
    def build_embedding_info(
        model_id: str,
        display_name: Optional[str] = None,
        description: Optional[str] = None,
    ) -> Dict[str, Any]:
        """
        构建 Embedding 模型的 info 字段
...
class QwenAPIServer:
    ...
    def __init__(self, debug: bool = False) -> None:
        ...
        # 模型能力构建器
        self.capabilities_builder = OpenWebUIModelCapabilities()
    ...
    def list_models(self) -> ModelsResponse:
        """
        列出模型 - 符合 OpenWebUI 规范

        返回格式遵循 OpenAI /v1/models 标准，
        同时在 info.meta.capabilities 中提供 OpenWebUI
        所需的模型能力标识字段:
        - chat: bool     - 是否支持聊天补全
        - image: bool    - 是否支持图像生成
        - tts: bool      - 是否支持文本转语音
        ...
        for model_id in self.available_models:
            ...
            is_vision = any(
                ...
                for keyword in ["vision", "vl", "omni", "multimodal"]
            )

            model_info = self.capabilities_builder.build_llm_info(
                model_id=model_id,
            ...
            models.append(
                ModelInfo(
                    ...
                    owned_by="qwen",
                    info=model_info,
                    permission=OpenWebUIModelCapabilities.build_model_permission(
                        created=created_time,
                        allow_sampling=True,
                    ),
                    root=model_id,
        ...
        for image_model in self.available_image_models:
            model_info = self.capabilities_builder.build_image_info(
        ...
        # TTS 模型
        tts_info = self.capabilities_builder.build_tts_info(
            model_id="tts-1",
            display_name="TTS-1",
        ...
        # ASR / Whisper 模型
        asr_info = self.capabilities_builder.build_asr_info(
            model_id="whisper-1",
        ...
        # 视频生成模型
        video_info = self.capabilities_builder.build_video_info(
            model_id="sora-2",
            display_name="Sora-2",
        ...
        # Embedding 模型
        embedding_info = self.capabilities_builder.build_embedding_info(
...
app = FastAPI(
    ...
    description=(
        ...
        "(Qwen FnCall 格式 + FIFO 公平调度 + OpenWebUI 适配)"
    ),
    version="6.0.0",
    lifespan=lifespan,
    docs_url="/docs",
...
async def list_models():
    """
    列出模型 - OpenAI / OpenWebUI 兼容

    返回符合 OpenWebUI 规范的模型列表，
    每个模型包含标准的 capabilities 能力标识:
    - chat: 聊天补全能力
    - image: 图像生成能力
    - tts: 文本转语音能力
    - embedding: 嵌入向量能力
...
async def get_model(model_id: str):
    ...
    created_time = int(time.time())
    builder = OpenWebUIModelCapabilities()
    ...
    if model_id in server.available_models:
        ...
        model_info = builder.build_llm_info(
            model_id=model_id,
            supports_vision=is_vision,
        )
    elif model_id in server.available_image_models:
        owned_by = "pollinations"
        model_info = builder.build_image_info(model_id=model_id)
    elif model_id == "tts-1":
        owned_by = "qwen"
        model_info = builder.build_tts_info(model_id=model_id)
    elif model_id == "whisper-1":
        owned_by = "openai"
        model_info = builder.build_asr_info(model_id=model_id)
    elif model_id == "sora-2":
        owned_by = "openai"
        model_info = builder.build_video_info(model_id=model_id)
    elif model_id == "qwen3-embedding:0.6b":
        owned_by = "qwen"
        model_info = builder.build_embedding_info(model_id=model_id)
    else:
        owned_by = "qwen"
        model_info = builder.build_llm_info(model_id=model_id)
...
def main() -> None:
    ...
    logger.info("支持 OpenWebUI 模型能力标识")
    logger.info(
        f"FnCall 格式: Qwen 原生 "
