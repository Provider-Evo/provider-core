function _updateOverviewNoticeAndBadges(summary, config) {
...
function _buildOverviewCards(summary, config) {
  ...
  cards.push(makeCard(t('render.platformFilter'), [
    ...
    t('render.proxyWhitelist') + ': ' + String(((config.adapter_proxy || {}).enable_adapters || []).join(', ') || ((config.platforms_proxy || {}).enabled_platforms || []).join(', ') || '-')
  ]));
  return cards;
}
...
function renderOverview(summary) {
  ...
  _updateOverviewNoticeAndBadges(summary, config);
  const cards = _buildOverviewCards(summary, config);
...
function syncModelFilterOptions(models) {
  ...
  const capabilities = Array.from(new Set((models || []).flatMap(function(model) {
    var caps = typeof resolveModelCapabilities === 'function'
...
function renderModels(models) {
  ...
  const filtered = state.models.filter(function(model) {
    ...
    const capabilities = typeof resolveModelCapabilities === 'function'
      ? resolveModelCapabilities(model)
      : (model.capabilities || {});
  ...
  modelGrid.innerHTML = filtered.slice(0, 200).map(function(model) {
    const caps = typeof resolveModelCapabilities === 'function'
      ? resolveModelCapabilities(model)
      : (model.capabilities || {});
    const capabilities = Object.keys(caps).filter(function(key) {
      return caps[key];
    }).join(', ') || '-';
    return [
