...
def _decode_data_uri(url: str) -> Optional[tuple]:
    """解码 data: URI，返回 (file_bytes, mime)，失败返回 None。"""
...
def _extract_image_part(part: Dict[str, Any]) -> Optional[tuple]:
    """从 image_url part 中提取 (file_bytes, filename)。"""
    url = part.get("image_url", {}).get("url", "")
    if not url.startswith("data:"):
        return None
    decoded = _decode_data_uri(url)
    if decoded is None:
        logger.warning("提取 image_url 失败")
        return None
    file_bytes, mime = decoded
    return file_bytes, "image.{}".format(_mime_to_ext(mime))


def _extract_video_part(part: Dict[str, Any]) -> Optional[tuple]:
    """从 video_url part 中提取 (file_bytes, filename)。"""
    url = part.get("video_url", {}).get("url", "")
    if not url.startswith("data:"):
        return None
    decoded = _decode_data_uri(url)
    if decoded is None:
        logger.warning("提取 video_url 失败")
        return None
    file_bytes, mime = decoded
    return file_bytes, "video.{}".format(_mime_to_ext(mime))


def _extract_audio_part(part: Dict[str, Any]) -> Optional[tuple]:
    """从 input_audio part 中提取 (file_bytes, filename)。"""
    data = part.get("input_audio", {}).get("data", "")
    if not data.startswith("data:"):
        return None
    decoded = _decode_data_uri(data)
    if decoded is None:
        logger.warning("提取 input_audio 失败")
        return None
    file_bytes, mime = decoded
    return file_bytes, "audio.{}".format(_mime_to_ext(mime))


def _extract_file_part(part: Dict[str, Any]) -> Optional[tuple]:
    """从 file part 中提取 (file_bytes, filename)。"""
    import base64 as _b64
    ...
    if not file_data.startswith("data:"):
        return None
    try:
        _header, b64_data = file_data.split(",", 1)
...
_PART_EXTRACTORS = {
    "image_url": _extract_image_part,
    "video_url": _extract_video_part,
    "input_audio": _extract_audio_part,
    "file": _extract_file_part,
}
    ...
    for msg in messages:
        ...
        for part in content:
            ...
            if extractor is None:
                continue
            extracted = extractor(part)
            if extracted is not None:
                upload_files.append(extracted)
