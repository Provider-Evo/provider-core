...
class GoogletranslatePlugin(ProviderPlugin):
    async def on_load(self) -> None:
        mod = importlib.import_module("provider_googletranslate.core.adaptercore")
        adapter = _find_adapter_class(mod)()
        attach_platform_adapter(self, adapter)


def create_plugin() -> GoogletranslatePlugin:
